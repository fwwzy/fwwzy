...
- (void)viewDidLoad
{
    ...
    self.fontSize = (int)[E_CommonManager fontSize];
    _pageIsAnimating = NO;
...
- (void)turnToClickMark:(E_Mark *)eMark{
    ...
    int showPage = (int)[self findOffsetInNewPage:NSRangeFromString(eMark.markRange).location];
...
{
    int currentPage = (int)[_pageViewController.viewControllers.lastObject currentPage];
...
- (void)fontSizeChanged:(int)fontSize
{
    ...
    _paginater.contentFont = self.fontSize;
    [_paginater paginate];
    int showPage = (int)[self findOffsetInNewPage:self.readOffset];
    [self showPage:showPage];
...
- (void)turnToClickSearchResult:(NSString *)chapter withRange:(NSRange)searchRange andKeyWord:(NSString *)keyWord{
    ...
    int showPage = (int)[self findOffsetInNewPage:searchRange.location - [[E_ReaderDataSource shareInstance] getChapterBeginIndex:[chapter integerValue]]];
    [self showPage:showPage];
...
{
    int pageCount = (int)_paginater.pageCount;
