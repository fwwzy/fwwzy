...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextAddLineToPoint(context, self.width-10, 75);
    CGContextAddLineToPoint(context, self.width-18, 71);
    CGContextAddLineToPoint(context, self.width-10, 75);
    CGContextAddLineToPoint(context, self.width-18, 79);
    CGContextStrokePath(context);
    
    //Create 5 strike line for see difference
    //And for +  and -
    int halfGraph = self.height/2;
    double yNumber = 0;
    for (int i = 0; i<2; i++) {
        for (int j = 0; j<5; j++) {
            halfGraph -= 15;
            ...
            CGContextAddLineToPoint(context, self.width-10, halfGraph);
            CGContextStrokePath(context);
    ...
    int middlelength = (self.width-40)/12;
    ...
    UILabel *yLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.width-100, 70, 90, 30)];
    ...
    if (_linesGraph) {
        ...
        yLabel.text = @"Month";
        double distanceNextYline = middlelength;
        double bizieLineAligm = 10;
        
        double nextX = middlelength+4;
