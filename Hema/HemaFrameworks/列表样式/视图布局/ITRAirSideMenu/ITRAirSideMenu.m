...
- (void)presentMenuViewContainerWithMenuViewController:(UIViewController *)menuViewController
{
    ...
    sublayerTransform.m34 = 1.0f / -UI_View_Width;
...
- (void)showLeftMenuViewController
{
    ...
    [UIView animateWithDuration:self.animationDuration animations:^{
        ...
        sublayerTransform1.m34 = 1.0f / -UI_View_Width;
...
- (void)panGestureRecognized:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateChanged) {
        ...
        if(newLocationPoint.x < 0)
            newLocationPoint.x = UI_View_Width + newLocationPoint.x;
        
        //track movement
        if((newLocationPoint.x >= 0 && newLocationPoint.x <= UI_View_Width))
        {
            [self setAnchorPoint:CGPointMake(1.0, 0.5) forView:_contentViewContainer];
            [self setAnchorPoint:CGPointMake(1.0, 0.5) forView:_contentViewController.view];
            
            //calculation of scale, angle & translate for content view based on pan position
            CGFloat contentViewScale = 1 + (_contentViewScaleValue - 1)* (newLocationPoint.x / UI_View_Width);
            CGFloat angle = (newLocationPoint.x - _lastPoint.x) * _contentViewRotatingAngle/UI_View_Width;
            CGFloat tranformX = newLocationPoint.x * _contentViewTranslateX/UI_View_Width;
            ...
            [UIView animateWithDuration:self.animationDuration animations:^{
                ...
                CGFloat menuAngle = _menuViewRotatingAngle - (newLocationPoint.x) * _menuViewRotatingAngle/UI_View_Width;
                CGFloat menuTransformValue = (_menuViewTranslateX * newLocationPoint.x/UI_View_Width) - _menuViewTranslateX - _menuViewContainer.frame.origin.x;
