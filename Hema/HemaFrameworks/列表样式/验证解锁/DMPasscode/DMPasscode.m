...
- (void)showPasscodeInViewController:(UIViewController *)viewController completion:(PasscodeCompletionBlock)completion {
    ...
    if ([context canEvaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics error:nil]) {
        [context evaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics localizedReason:@"请验证" reply:^(BOOL success, NSError* error) {
...
- (void)openPasscodeWithMode:(int)mode viewController:(UIViewController *)viewController {
    ...
    if (_mode == 0) {
        [_passcodeViewController setInstructions:@"输入新密码"];
        [_passcodeViewController.navigationItem setNewTitle:@"设置"];
    } else if (_mode == 1) {
        [_passcodeViewController setInstructions:@"输入解锁密码"];
        [_passcodeViewController.navigationItem setNewTitle:@"验证"];
    }
}

- (void)closeAndNotify:(BOOL)success {
    [_passcodeViewController.input resignFirstResponder];
...
- (void)enteredCode:(NSString *)code {
    if (_mode == 0) {
        if (_count == 0) {
            _prevCode = code;
            [_passcodeViewController setInstructions:@"请再次输入"];
        ...
        if ([code isEqualToString:[[DMKeychain defaultKeychain] objectForKey:KEYCHAIN_NAME]]) {
            ...
            [_passcodeViewController setErrorMessage:[NSString stringWithFormat:@"还剩%i机会", 2 - _count]];
            [_passcodeViewController reset];
