...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    NSLog(@"问题：%@",[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
    
    NSString *errorStr = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    //NSString *myStr = @"<stream:error xmlns:stream=\"http://etherx.jabber.org/streams\"><conflict xmlns=\"urn:ietf:params:xml:ns:xmpp-streams\"/></stream:error>";
    NSString *myStr = @"conflict";
    NSRange range;
    range = [errorStr rangeOfString:myStr];
    if (range.location != NSNotFound)
    {
        [HemaFunction xfuncGetAppdelegate].isDuoDian = YES;
    }
