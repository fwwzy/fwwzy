...
#define ITEM_WIDTH UI_View_Width
...
static CGFloat SWITCH_FOCUS_PICTURE_INTERVAL = BB_ScrollTime;
...
- (void)setupViews
{
    ...
    _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.frame.size.height -16-10, UI_View_Width, 10)];
...
-(void)addImageViews:(NSArray *)aImageItems{
    ...
    float space = 0;
    CGSize size = CGSizeMake(UI_View_Width, 0);
    for (int i = 0; i < aImageItems.count; i++) {
        SGFocusImageItem *item = [aImageItems objectAtIndex:i];
        HemaImgView *imageView = [[HemaImgView alloc] initWithFrame:CGRectMake(i * _scrollView.frame.size.width+space, space, _scrollView.frame.size.width-space*2, _scrollView.frame.size.height-2*space-size.height)];
        //加载图片
        imageView.backgroundColor = i%2?[UIColor redColor]:[UIColor blueColor];
        imageView.contentMode = UIViewContentModeScaleAspectFill;
        
        //做处理 item.image 存储的是url地址字符串
        [SystemFunction cashImgView:imageView url:item.image firstImg:@"R图片默认背景.png"];
